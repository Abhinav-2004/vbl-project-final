...
app.post('/profileSetting', async(req,res)=>{
  ...
  if(token){
    jwt.verify(token, jwtSecret, {},async (err,userData)=>{
      if(err) throw err;
      const {name,email,_id,designation,employeeID,department} = await User.findById(userData.id);
      res.json({name,email,_id,designation,employeeID,department});
      var myquery = { _id:_id };
      var newvalues = {$set: {name: req.body.name, designation:req.body.designation,department:req.body.department} };
